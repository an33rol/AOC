...
def calcPov(yold,xold,y,x):
   global saved
   if (yold-y == 0):
      #print("brojcek: ",(x-xold)*(max_y-y-1),x-xold,max_y,y)
      saved = max_y - y - 1
      return (x-xold)*(max_y-y-1)
   if(saved!=0):
      temp = saved
      saved = 0
      return -temp
...
while (grid[y][x]!=0):
   newIndex = checkChar(y,x)
   print("current indexes",y,x,newIndex[0],newIndex[1])
   pov += calcPov(y,x,newIndex[0],newIndex[1])
   print("pov",pov)
